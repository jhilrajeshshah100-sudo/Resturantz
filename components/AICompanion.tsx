  ...
  const handleSend = async (textOverride?: string) => {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: apiHistory,
        config: {
          systemInstruction: `You are Evelyn, the elegant and warm digital concierge for the 'Farm & Fork' wedding. 
          Your tone is sophisticated, welcoming, and helpful. Avoid overly technical jargon.
          
          WEDDING DETAILS:
          - Venue: The Vineyard Valley Estate.
          - Dress Code: 'Vineyard Chic' (Elegant summer attire, block heels for grass, light suits).
          ...
          BEHAVIOR:
          - Provide clear, helpful, and charming answers.
          - Use emojis like 🥂, ✨, 🍷 gracefully.
          - If asked about weather or local info, use the googleSearch tool to provide real-time updates.`,
          tools: [{ googleSearch: {} }]
        }
      });
      ...
      if (error.message?.includes("API_KEY_INVALID") || error.message?.includes("Requested entity was not found")) {
        setMessages(prev => [...prev, { role: 'model', text: "I'm having trouble connecting to my system. Please select a valid API Key to continue. ✨" }]);
        handleOpenKeySelector();
      } else {
        setMessages(prev => [...prev, { role: 'model', text: "I encountered a small hiccup. Could you ask me that once more? 🥂" }]);
      }
    } finally {
      setIsLoading(false);
    }
      ...
      <div className="relative w-full max-w-md bg-[#16212B] rounded-t-[32px] shadow-2xl flex flex-col h-[75vh] pointer-events-auto border-t border-white/10 animate-slide-up">
          ...
          {messages.map((m, i) => (
            <div key={i} className={`flex flex-col ${m.role === 'user' ? 'items-end' : 'items-start'}`}>
              <div className={`max-w-[85%] p-4 rounded-2xl ${
                m.role === 'user' ? 'bg-primary text-white rounded-tr-none shadow-md' : 'bg-white/5 text-gray-200 rounded-tl-none border border-white/10 shadow-sm'
              }`}>
                <p className="text-sm leading-relaxed">{m.text}</p>
                {m.sources && m.sources.length > 0 && (
                  <div className="mt-3 pt-3 border-t border-white/10 space-y-2">
                    <p className="text-[9px] uppercase font-black text-gray-500 tracking-tighter">Sources:</p>
                    <div className="flex flex-wrap gap-2">
                      {m.sources.map((s, idx) => (
                        <a key={idx} href={s.uri} target="_blank" rel="noopener noreferrer" className="text-[10px] bg-primary/10 text-primary px-2 py-1 rounded hover:bg-primary/20 transition-colors">
                          {s.title}
                        </a>
                      ))}
                    </div>
        ...
        <div className="p-6 bg-[#16212B] border-t border-white/5">
          ...
          <div className="mt-4 text-center">
            <button onClick={handleOpenKeySelector} className="text-[10px] text-gray-600 hover:text-primary transition-colors uppercase tracking-widest font-bold">
